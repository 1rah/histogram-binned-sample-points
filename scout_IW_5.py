...
def read_arguments():
   ...
   parser.add_argument('-i',action='store_true', help='run interactive mode: user selected start point')
    ...
    if args.i:
        ...
        def onclick(event):
            ...
            if event.dblclick:
                ...
                print('Path points:', path_to_line(path))
        ...
        p0 = (indice_data.shape[1]//2, indice_data.shape[0]//2)
        print('start point (p0):',p0)
        path, class_list = get_path(p0, px_list)
        print('Path points:', path_to_line(path))
